...
const MyCataLog = () => {
  ...
  const handleActiveSubmit = async () => {
    ...
    // 🧮 Count how many chemicals are marked as active
    const activeCount = activeChemicalList.filter(
      (chem) => chem.active_chemicals === "active"
    ).length;

    console.log("Active Count:", activeCount, "Limit:", catalogLimit);
    ...
    // 🚨 Check if active chemicals exceed plan limit
    if (activeCount > catalogLimit) {
      setPackageExpire(true);
      toast.error(
        `You can activate only ${catalogLimit} chemicals with your current plan.`,
        {
          position: toast.POSITION.BOTTOM_RIGHT,
          autoClose: 2000,
        }
      );
      return; // ❌ Stop here — don’t send update request
    }

    // ✅ Proceed to update backend if within limit
    setLoading(true);
    try {
      const token = `Bearer ${localStorage.getItem("chemicalToken")}`;
